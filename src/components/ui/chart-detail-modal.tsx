...
import { GradientRoundedAreaChart, type ChartDataPoint } from "@/components/ui/gradient-rounded-chart";
...
const StatBadge = ({ change, changeType }: StatBadgeProps) => {
  ...
  return (
    <div className={cn("inline-flex items-center rounded-md px-2 py-1 text-xs font-medium", styles)}>
...
const ChartSection: React.FC<ChartSectionProps> = ({ data, config, xAxisKey = "date", dataKeys, chartId }) => {
  return (
    <div className="mt-4">
...
export const ChartDetailModal: React.FC<ChartDetailModalProps> = ({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={title} size={sideBySide ? "2xl" : "lg"}>
      {/* Header subtext and change badge */}
      {(subtitle || change) && (
        <div className="flex items-center justify-between mb-3">
          {subtitle ? (
            <p className="text-sm text-muted-foreground dark:text-muted-foreground/85">{subtitle}</p>
          ) : <span />}
      ...
      {metadata.length > 0 && (
        <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
          {metadata.map((m) => (
            <div key={m.label} className="flex items-center justify-between rounded-md bg-transparent px-1 py-1">
